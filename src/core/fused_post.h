    ...
    for (size_t idx : mask_.epi_wound) {
      ...
      if (do_biofilm) {
        ...
        if (stratum_val < 1.0) {
          ...
          if (bf > 1e-10) {
            real_t delta = bf_growth * bf * (1.0 - bf / bf_K);
            if (delta > 1e-10) {
      ...
      if (do_mmp) {
        ...
        if (mmp_val > 1e-10) {
          ...
          if (col_val > 1e-10) {
            real_t degrade = std::min(col_val, mmp_val * mmp_col_degrade);
            if (degrade > 1e-10) {
          ...
          if (do_fn && fn_grid) {
            ...
            if (fn_val > 1e-10) {
              real_t fn_loss = std::min(fn_val, mmp_val * mmp_fn_degrade);
              if (fn_loss > 1e-10) {
          ...
          if (do_elastin_degrade && elastin_grid) {
            ...
            if (el_val > 1e-10) {
              real_t el_loss = std::min(el_val, mmp_val * mmp_el_degrade);
              if (el_loss > 1e-10) {
          ...
          if (do_fibrin_degrade && fibrin_grid) {
            ...
            if (fb_val > 1e-10) {
              real_t fb_loss = std::min(fb_val, mmp_val * mmp_fb_degrade);
              if (fb_loss > 1e-10) {
      ...
      if (do_fn) {
        ...
        if (sv < 1.0 && fn_serum > 1e-10) {
          ...
          fn_grid->ChangeConcentrationBy(
              sidx(fn_grid, idx), fn_serum * gate * coarse_w);
        }
      }
