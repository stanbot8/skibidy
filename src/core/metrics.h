  ...
  void operator()() override {
    ...
    if (!ofs_) {
      ...
      *ofs_ << "step,time_h,time_days,n_agents,n_stem,n_ta,n_g0,"
            ...
            << "n_fibroblasts,n_myofibroblasts,mean_tgfb_wound,mean_collagen_wound,"
    ...
    int n_fibroblasts = 0, n_myofibroblasts = 0;
    int n_tumor_cells = 0;
    ...
    rm->ForEachAgent([&](Agent* agent) {
      ...
      if (fibro) {
        n_fibroblasts++;
        if (fibro->GetFibroblastState() == kMyofibroblast) {
          n_myofibroblasts++;
        }
        return;
      }
    ...
    if (sp->wound_enabled) {
      ...
      if (sp->ph_enabled) {
        ph_grid = rm->GetDiffusionGrid(fields::kPH);
      }
