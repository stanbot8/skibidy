  ...
  void Run(Agent* agent) override {
    ...
    switch (cell->GetCyclePhase()) {
      case kG1: {
        ...
        // Inflammation gating: Hill-function dose-response (standard
        // biochemical kinetics).  Cells progressively slow with rising
        // inflammation but retain residual activity -- no hard cutoff.
        ...
        real_t infl_factor = (K * K) / (K * K + eff_infl * eff_infl);
        p *= infl_factor;
