...
_REF_DIR = os.path.join(os.path.dirname(__file__), "data", "consensus")
...
def detect_modules(sim):
    """Return (has_wound, has_fibroblast, has_tumor) booleans."""
    ...
    return has_wound, has_fibroblast, has_tumor
...
def validate_wound(sim, sim_days, condition="normal"):
    ...
    if condition == "diabetic":
        ref_closure = load_csv(os.path.join(_REF_DIR, "diabetic_closure_kinetics.csv"))
        ref_infl = load_csv(os.path.join(_REF_DIR, "diabetic_inflammation_timecourse.csv"))
        ref_immune = load_csv(os.path.join(_REF_DIR, "diabetic_immune_cell_kinetics.csv"))
    else:
        ref_closure = load_csv(os.path.join(_REF_DIR, "closure_kinetics_punch_biopsy.csv"))
        ref_infl = load_csv(os.path.join(_REF_DIR, "inflammation_timecourse.csv"))
        ref_immune = load_csv(os.path.join(_REF_DIR, "immune_cell_kinetics.csv"))
...
def validate_fibroblast(sim, sim_days):
    ...
    ref_myofib = load_csv(os.path.join(_REF_DIR, "myofibroblast_kinetics.csv"))
    ref_collagen = load_csv(os.path.join(_REF_DIR, "collagen_deposition.csv"))
...
def validate_tumor(sim, sim_days):
    ...
    ref_td = load_csv(os.path.join(_REF_DIR, "tumor_doubling_time.csv"))
    ref_ki67 = load_csv(os.path.join(_REF_DIR, "tumor_proliferation_index.csv"))
...
def plot_fibroblast_panels(r, sim_days, axes):
    """Draw 2 fibroblast panels into a (2,) or (1,2) axes array."""
    ax = axes[0]
...
def print_summary(wound=None, fibroblast=None, tumor=None):
