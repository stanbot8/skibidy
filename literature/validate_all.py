...
from lib import (load_csv, detect_condition, detect_modules, validate_wound,
                 validate_fibroblast, validate_tumor, plot_wound_panels,
                 plot_fibroblast_panels, plot_tumor_panels, print_summary)
...
def main():
    ...
    has_wound, has_fibroblast, has_tumor = detect_modules(sim)
    ...
    if nrows > 0:
        ...
        if wound_r:
            ...
            row += 2
        if fibro_r:
            plot_fibroblast_panels(fibro_r, sim_days, axes[row])
            row += 1
        if tumor_r:
